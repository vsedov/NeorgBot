    ...
    async def youtube(self, ctx: Context, *, query: str) -> None:
        ...
        with requests.get(url + query) as response:
            ...
            fuzzy_matches = process.extract(
                query, match, scorer=fuzz.token_set_ratio, limit=5, score_cutoff=20
            )

            option_list = {"https://www.youtube.com/watch?v=" + match: "Perfect Match"}
            for i in fuzzy_matches:
                option_list["https://www.youtube.com/watch?v=" + i[0]] = i[1]

        embed = discord.Embed(
            title="YouTube Search", description=query, colour=0xFF0000
        )
        ...
        await ctx.send(
            embed=discord.Embed(title="Playing", description=url, colour=0x00FF00)
        )
